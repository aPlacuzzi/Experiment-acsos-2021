...
public final class Engine<T, P extends Position<? extends P>> implements Simulation<T, P> {
    ...
    public void reactionAdded(final Reaction<T> target) {
        checkCaller();
        afterExecutionUpdates.add(new ReactionAddition(target));
    }

    public void reactionRemoved(final Reaction<T> target) {
        checkCaller();
        afterExecutionUpdates.add(new ReactionRemoval(target));
    }
